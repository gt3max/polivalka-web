...
| Limit | Value | Purpose |
|-------|-------|---------|
| Daily soft limit | max_water_day_ml | v1.0.173+: stops pump until midnight when reached (`enforce_daily_soft_limit=1`, default ON). Pre-v1.0.173: warning only. |
| Daily hard limit | max_water_day_ml × multiplier | Immediate stop on PULSE complete (was deferred-flag pre-v1.0.173) |
| Per-session strict cap | max_ml_per_session_strict (v1.0.173+) | Hard cap per single watering session — protects against soil channeling overflow (ref: BB00C1 incident 2026-05-04, ~960 ml in 30 cycles) |
| Global hard cap | 2000 ml/day | Flood protection (hardcoded in firmware) |
